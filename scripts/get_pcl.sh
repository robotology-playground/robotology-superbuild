...
#check if WALKMAN_ROOT is defined
if [ -n "$WALKMAN_ROOT" ]
then

    mkdir -p $WALKMAN_ROOT/external/
    cd $WALKMAN_ROOT/external/
    #clone the pcl repo
    git clone https://github.com/PointCloudLibrary/pcl.git -b pcl-1.7.1 --depth 1 PCL

    mkdir -p $WALKMAN_ROOT/external/PCL/build
    cd $WALKMAN_ROOT/external/PCL/build
    rm CMakeCache.txt
    ...
    echo "Please source walkman-env.bash before running this script."
    exit
fi
